...
- **Headless + skinnable UI** — `useToolbar`/`useInsertBar` expose the live buttons; `EditorToolbar`, `BubbleToolbar` and `InsertToolbar` are thin, overridable skins over them. `useZoom` ships the zoom state/policy for the `zoom` prop.
